...
private:
    struct Function {
        ...
        map<string, Node*> env;     // 沒有使用
    };

    map<Node*, Function> lambdaTable;
    map<string, Node*> symbolTable;  // x1 : 5   // x2 : 10    //  x3 : (10 20 30)
...
public:
    ...
    Node* Evalulated(Node* exp) 
    {
        ...
        if( exp->left && (exp->left->left||exp->left->right) || ( primitiveNames.count(exp->left->token.str)==0 && functionTable.count(exp->left->token.str)==0 ))
        {
            ...
            if(primitiveNames.count(op->token.str) && op->token.type!=Func)
            {
                errornode=op;
    ...
    Node* EvalLambdaFunc(Node* args, Node* lambda)
    {
        int arguments = 0;
        Function funclambda = lambdaTable[lambda];
        Node* argument = args;
        while (argument && argument->token.type != NIL) 
        {   // 考慮過參數為0的情況
            ++arguments;
    ...
    Node* EvalLambda(Node* args, Node*head)
    {
        ...
        Node* lam = new Node();
        lambdaTable[lam] = f;      //沒有名字，不知道找誰，所以我用記憶體位置
    ...
    Node* FindResult(Node* args, Node* head)
    {
        ...
        while(argument && argument->token.type != NIL)
        {
            ...
            {
                errormessage = e.what();
                if(errormessage != "ERROR (no return value) : " && errormessage != "ERROR (unbound parameter) : ")
                {
                    throw runtime_error(e);
                }
                else if(argument->right &&argument->right->token.type == NIL)
                {
                    throw runtime_error(e);
                }
                else
    ...
    Node* EvalBegin(Node* args)
    {
        ...
        while(argumnet&&argumnet->token.type != NIL)
        {
            ...
            catch(runtime_error& e)
            {
                ...
                else if(errormessage == "ERROR (no return value) : " && ( argumnet->right && argumnet->right->token.type==NIL ) )
                {
                    throw runtime_error("ERROR (unbound parameter) : ");
                }
                else
                {
                    argumnet = argumnet->right;
    ...
    Node* EvalPlus(Node* args)
    {
        ...
        {
            Node* num = nullptr;
            if(argument->left->left)
            {
                Evalulated(argument->left->left);
                if(argument->left->left->token.type == Func)
                    argument->left->left->token.type = SYMBOL;
            }
            try 
            {
                num = Evalulated(argument->left);
            }
            catch(runtime_error& e)
            {
                string errormessage = e.what();
    ...
    Node* EvalList(Node* args)
    {
        ...
        Node* run = new Node();
        Node* root = run;
        while(temp && temp->token.type!=NIL)
        {
            run->left = Evalulated(temp->left);
            run->right = new Node();
            run = run->right;
            temp=temp->right;
        }
    ...
    {   // (define (myfunction x1 x2 x3 ) x1 x2 (+ x1 x2) )
        // (myfunction 10 20 30)
        // 檢查參數數量是否一致
        ...
        // 把參數綁定
        map<string, Node*> state = symbolTable;      // 原始的狀態
        map<string, Node*> thenew = symbolTable;     // 改變中
        param = args;
        for(int n = 0 ; n < myFunc.parameters.size() ;n++)
        {
            try
            {
            ...
            catch(runtime_error& e)
            {
                symbolTable = state;
                string errormessage = e.what();
                if(errormessage == "ERROR (no return value) : ")
                    throw runtime_error("ERROR (unbound parameter) : ");
                else
                    throw runtime_error(e);
            }
