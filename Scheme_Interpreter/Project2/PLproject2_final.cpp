...
public:
    ...
    Node* Evalulated(Node* exp) 
    {
        ...
        Node* op = nullptr;
        if(exp->left && (exp->left->left|| exp->left->right) || primitiveNames.count(exp->left->token.str)==0)
            op = Evalulated(exp->left);  // operation
        else
            op = exp->left;
        Node* args = exp->right;
    ...
    Node* EvalIf(Node* args, Node* head)
    {
        ...
        Node* value = Evalulated(args->left); // 判斷式
    ...
    Node* EvalCond(Node* args, Node* head)
    {
        ...
        argument = args;
        // (cond (nil 10 ) (nil 100 200 300) )   沒有可以回傳的東西，這種格式會錯
        EvalConditions(argument, head); // format
        Node* root = nullptr;
        while (argument && argument->token.type != NIL)
        {
