...
public:
    ...
    void InitPrimitives() 
    {
        ...
        vector<string> primitives = 
        {
            ...
            "eqv?", "equal?", "begin", "if", "cond", "clean-environment", "exit"
        };
    ...
    void EvalConditions(Node* args, Node* head)
    {
        ...
        while(argument && argument->token.type != NIL)
        {
            if(!argument->left || !argument->left->left || !argument->left->right || argument->left->right->token.type==NIL)
            {
                errornode = head;
                throw runtime_error("ERROR (COND format) : ");
            }

            if(argument->left->left->token.str == "else" && argument->right->token.type == NIL)
            {
                argument->left->left->token.type = TRUE;
                argument->left->left->token.str = "#t";
                return ;
            }
            else 
            {
                argument->left->left = Evalulated(argument->left->left);    
            }

            if(argument->left->left->token.type!=NIL)
            {
                return ;
            }
    ...
    Node* FindResult(Node* args, Node* head)
    {
        ...
        while(argument && argument->token.type != NIL)
        {
            argument->left = Evalulated(argument->left);
    ...
    Node* EvalCond(Node* args, Node* head)
    {
        ...
        if (arguments < 1)
            throw runtime_error("ERROR (incorrect number of arguments) : cond\n");
    ...
    while(true)
    {
        ...
        if(syntax_anaylize.Is_SEXP()) {
            ...
            {            
                ...
                if(root->left->token.str=="exit" && root->right->token.type==NIL)
                {
                    cout << endl;
                    break;
                }
